...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        // ✅ SPACE = RAPID DROP BURST (tap only)
        if (IsKeyPressed(KEY_SPACE))
        {
            spacePressed = true;
            spaceBurstTimer = currentTime;
        }

        // Burst lasts 0.25 seconds only
        if (spacePressed)
        {
            if (currentTime - spaceBurstTimer <= 0.25)
            {
                if (EventTriggered(0.05)) // very fast falling
                {
                    game.MoveBlockDown();
                }
            }
